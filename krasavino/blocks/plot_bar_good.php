...
if ($acct_yr != 'Все годы') {
	...
	$total_title = "ТОП-15 товаров за $acct_yr год";
} else {
	$where_year = '';
	$total_title = 'ТОП-15 товаров за все годы';
}
...
$sql_select = array("SELECT count(*) as $select[0], name FROM shops $where_year GROUP BY name ORDER BY $select[0] DESC LIMIT 15",
			"SELECT SUM(amount) as $select[1], name FROM shops $where_year GROUP BY name ORDER BY $select[1] DESC LIMIT 15");
