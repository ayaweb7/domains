...
// Когда выбран анализ только в группе:
if ($_POST['name'] == 'Товар не выбран' AND ($_GET['gruppaSelected'])) { //  AND $_POST['gruppa'] != 'Не выбрана')
	$gruppa = $_POST['gruppaSelected'];
	
	if ($acct_yr != 'Все годы') {
	$where_year = "WHERE YEAR(date)='$acct_yr' AND gruppa='$gruppa'";
	$total_title = "$gruppa за $acct_yr год по месяцам";
	} else {
		$where_year = "WHERE gruppa='$gruppa'";
		$total_title = "$gruppa по месяцам за все годы";
	}
	
// Когда для анализа выбраны группа и товар одновременно:	
} elseif ($_POST['name'] != 'Товар не выбран' AND (isset($_GET['gruppaSelected']))) {
	$name = $_POST['name'];
	//$gruppa = $_GET['gruppaSelected'];
	
	if ($acct_yr != 'Все годы') {
	$where_year = "WHERE YEAR(date)='$acct_yr' AND gruppa='$gruppa' AND name='$name'";
	$total_title = "$name из категории: $gruppa за $acct_yr год по месяцам";
	} else {
		$where_year = "WHERE gruppa='$gruppa' AND name='$name'";
		$total_title = "$name из категории: $gruppa по месяцам за все годы";
	}
}
