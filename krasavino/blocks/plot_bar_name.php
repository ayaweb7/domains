...
if ($acct_yr != 'Все годы') {
	$where_year = "WHERE YEAR(date)='$acct_yr' AND name='$name'";
	$total_title = "$name за $acct_yr год по месяцам";
} else {
	$where_year = "WHERE name='$name'";
	$total_title = "$name по месяцам за все годы";
}
