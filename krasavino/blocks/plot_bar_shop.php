...
if ($acct_yr != 'Все годы') {
	...
	$total_title = "ТОП-20 магазинов за $acct_yr год";
} else {
	$where_year = '';
	$total_title = 'ТОП-20 магазинов за все годы';
}
...
$sql_select = array("SELECT count(*) as $select[0], shop FROM shops $where_year GROUP BY shop ORDER BY $select[0] DESC LIMIT 20",
			"SELECT SUM(amount) as $select[1], shop FROM shops $where_year GROUP BY shop ORDER BY $select[1] DESC LIMIT 20");
...
for ($i = 0; $i <= 1; ++$i)
{
	...
	$graph[$i]->SetMargin($lm, $rm, 40, 200);
