...
if ($acct_yr != 'Все годы') {
	$where_year = "WHERE YEAR(shops.date)='$acct_yr'";
	$total_title = "Распределение покупок за $acct_yr год по городам";
} else {
	$where_year = "";
	$total_title = "Общее распределение покупок по городам за все годы";
}
...
$sql_select = array("SELECT count(*) as $select[0], shops.shop, store.town FROM shops LEFT JOIN store ON shops.shop = store.shop $where_year
						GROUP BY town ORDER BY $select[0] DESC LIMIT 20",
					"SELECT SUM(shops.amount) as $select[1], shops.shop, store.town FROM shops LEFT JOIN store ON shops.shop = store.shop $where_year
						GROUP BY town ORDER BY $select[1] DESC LIMIT 20");
